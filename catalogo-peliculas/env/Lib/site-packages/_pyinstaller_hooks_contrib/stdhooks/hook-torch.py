...
if is_module_satisfies("PyInstaller >= 6.0"):
    ...
    if is_linux:
        ...
        try:
            ...
            logger.warning(
                "hook-torch: failed to infer NVIDIA CUDA hidden imports!", exc_info=True
            )
            nvidia_hiddenimports = []
        logger.info(
            "hook-torch: inferred hidden imports for CUDA libraries: %r",
            nvidia_hiddenimports,
        )
    ...
    if is_win:
        ...
        def _collect_mkl_dlls():
            ...
            mkl_binaries = []
            logger.info(
                "hook-torch: collecting DLLs from MKL and its dependencies: %r",
                requirements,
            )
            ...
            logger.info(
                ...
                sorted(
                    [os.path.basename(src_name) for src_name, dest_name in mkl_binaries]
                ),
            )
            return mkl_binaries
