...
if uses_openssl3:
    ...
    if is_module_satisfies("PyInstaller >= 6.0"):
        ...
        if compat.is_win:
            ...
            openssl_lib = bindepend.resolve_library_path(
                lib_name, search_paths=[compat.base_prefix]
            )
            ...
            openssl_lib = bindepend._resolve_library_path_in_search_paths(
                lib_name, search_paths=[base_prefix_lib_dir]
            )
    ...
    if openssl_lib:
        logger.debug(
            "hook-cryptography: OpenSSL shared library location: %r", openssl_lib
        )
        ...
        if ossl_modules_dir.is_dir():
            logger.debug(
                "hook-cryptography: collecting OpenSSL modules directory: %r",
                str(ossl_modules_dir),
            )
