...
class PyiModuleGraph(ModuleGraph):
    ...
    def _reset(self, user_hook_dirs):
        ...
        for uhd in self._user_hook_dirs:
            ...
            try:
                ...
                logger.error(
                    "Unable to read run-time hooks from %r: %s" % (uhd_path, e)
                )
                continue
        ...
        while True:
            ...
            for module_name, module_hooks in self._hooks.items():
                ...
                for module_hook in module_hooks:
                    ...
                    self._module_collection_mode.update(
                        module_hook.module_collection_mode
                    )
            ...
            if excluded_imports:
                ...
                if level > ABSOLUTE_IMPORT_LEVEL:
                    if isinstance(source_module, Package):
                        ...
                        base_module_name = ".".join(
                            source_module.identifier.split(".")[:-1]
                        )
                    ...
                    if level > 1:
                        base_module_name = ".".join(
                            base_module_name.split(".")[: -(level - 1)]
                        )
                else:
                    base_module_name = target_module_partname
                ...
                def _exclude_module(module_name, excluded_imports):
                    ...
                    for excluded_import in excluded_imports:
                        ...
                        match = (
                            module_name_parts[: len(excluded_import_parts)]
                ...
                excluded_import_rule = _exclude_module(
                    base_module_name, excluded_imports
                )
                ...
                if target_attr_names:
                    ...
                    for target_attr_name in target_attr_names:
                        ...
                        excluded_import_rule = _exclude_module(
                            submodule_name, excluded_imports
                        )
                        if excluded_import_rule:
                            logger.debug(
    ...
    def _safe_import_module(self, module_basename, module_name, parent_package):
        ...
        if module_name in self._hooks_pre_safe_import_module:
            ...
            for hook in self._hooks_pre_safe_import_module[module_name]:
                ...
                logger.info(
                    "Processing pre-safe-import-module hook %r from %r",
                    hook_basename,
                    hook_path,
                )
                hook_module_name = (
                    "PyInstaller_hooks_pre_safe_import_module_"
                    + module_name.replace(".", "_")
                )
                hook_module = importlib_load_source(
                    hook_module_name, hook.hook_filename
                )
                ...
                if not hasattr(hook_module, "pre_safe_import_module"):
                    raise NameError(
                        "pre_safe_import_module() function not defined by hook %r."
                        % hook_module
                    )
    ...
    def _find_module_path(self, fullname, module_name, search_dirs):
        ...
        if fullname in self._hooks_pre_find_module_path:
            ...
            for hook in self._hooks_pre_find_module_path[fullname]:
                ...
                hook_path, hook_basename = os.path.split(hook.hook_filename)
                logger.info(
                    "Processing pre-find-module-path hook %r from %r",
                    hook_basename,
                    hook_path,
                )
                hook_fullname = (
                    "PyInstaller_hooks_pre_find_module_path_"
                    + fullname.replace(".", "_")
                )
                ...
                if not hasattr(hook_module, "pre_find_module_path"):
                    raise NameError(
                        "pre_find_module_path() function not defined by hook %r."
                        % hook_module
                    )
    ...
    def get_importers(self, name):
        ...
        importers = (
            importer.identifier for importer in importers if importer is not None
        )
        ...
        for mod_name, path, typecode in temp_toc:
            ...
            if mod_name in self._available_rthooks:
                ...
                for abs_path in self._available_rthooks[mod_name]:
                    ...
                    logger.info(
                        "Including run-time hook %r from %r", hook_basename, hook_path
                    )
    ...
    def make_hook_binaries_toc(self) -> list:
        ...
        for node in self.iter_graph(start=self._top_script_node):
            ...
            for dest_name, src_name in self._additional_files_cache.binaries(
                module_name
            ):
...
def get_bootstrap_modules():
    ...
    loader_mods += [
        (
            "pyimod01_archive",
            os.path.join(loaderpath, "pyimod01_archive.py"),
            "PYMODULE",
        ),
        (
            "pyimod02_importers",
            os.path.join(loaderpath, "pyimod02_importers.py"),
            "PYMODULE",
        ),
    ...
    if is_win:
        loader_mods.append(
            (
                "pyimod04_pywin32",
                os.path.join(loaderpath, "pyimod04_pywin32.py"),
                "PYMODULE",
            )
        )
    ...
    loader_mods.append(
        (
            "pyiboot01_bootstrap",
            os.path.join(loaderpath, "pyiboot01_bootstrap.py"),
            "PYSOURCE",
        )
    )
    return loader_mods
