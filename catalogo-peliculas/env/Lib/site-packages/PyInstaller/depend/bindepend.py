...
def binary_dependency_analysis(binaries, search_paths=None):
    ...
    for dest_name, src_name, typecode in output_toc:
        ...
        for dep_name, dep_src_path in get_imports(src_name, search_paths):
            logger.debug(
                "Processing dependency, name: %r, resolved path: %r",
                dep_name,
                dep_src_path,
            )
            ...
            if not dylib.include_library(dep_src_path):
                logger.debug(
                    "Skipping dependency %r due to global exclusion rules.",
                    dep_src_path,
                )
                continue
            ...
            if dep_src_path in processed_dependencies:
                logger.debug(
                    "Skipping dependency %r due to prior processing.", str(dep_src_path)
                )
                continue
            ...
            logger.debug(
                "Collecting dependency %r as %r.", str(dep_src_path), str(dep_dest_path)
            )
            ...
            if not compat.is_win and dep_dest_path.parent != pathlib.PurePath("."):
                ...
                output_toc.append(
                    (str(dep_dest_path.name), str(dep_dest_path), "SYMLINK")
                )
...
def _get_imports_macholib(filename, search_paths):
    ...
    def _resolve_using_path(lib):
        ...
        if os.path.isabs(lib):
            if (
                ...
                and _dyld_shared_cache_contains_path(lib)
            ):
                return lib
    ...
    for referenced_lib in referenced_libs:
        ...
        if referenced_lib.startswith("@rpath"):
            lib = os.path.join(
                *referenced_lib.split(os.sep)[1:]
            )  # Remove the @rpath/ prefix
            ...
            for run_path in run_paths:
                ...
                if lib_path.startswith(("@executable_path", "@loader_path")):
                    ...
                    lib_path = _resolve_using_loader_path(
                        lib_path, bin_path, python_bin_path
                    )
                else:
            ...
            if referenced_lib.startswith(("@executable_path", "@loader_path")):
                resolved_lib = _resolve_using_loader_path(
                    referenced_lib, bin_path, python_bin_path
                )
            else:
        ...
        if not resolved_lib:
            resolved_lib = _resolve_library_path_in_search_paths(
                os.path.basename(
                    referenced_lib
                ),  # Search for basename of the referenced name.
                search_paths,
            )
...
def resolve_library_path(name, search_paths=None):
    ...
    if compat.is_unix:
        ...
        win_search_paths = [
            path for path in compat.getenv("PATH", "").split(os.pathsep) if path
        ]
...
def get_python_library_path():
    ...
    # linked.
    imported_libraries = get_imports(
        compat.python_executable
    ...
    if compat.is_win and any(
        [
            os.path.normcase(lib_name) == "version.dll"
            for lib_name, _ in imported_libraries
        ]
    ):
