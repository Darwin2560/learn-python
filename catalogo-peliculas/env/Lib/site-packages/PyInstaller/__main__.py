...
def generate_parser() -> _PyiArgumentParser:
    ...
    parser._add_options(
        PyInstaller.building.build_main.__add_options, name="build_main"
    )
...
def run(pyi_args: list | None = None, pyi_config: dict | None = None):
    ...
    try:
        ...
        logger.info(
            "PyInstaller: %s, contrib hooks: %s", __version__, contrib_hooks_version
        )
        logger.info(
            "Python: %s%s",
            platform.python_version(),
            " (conda)" if compat.is_conda else "",
        )
