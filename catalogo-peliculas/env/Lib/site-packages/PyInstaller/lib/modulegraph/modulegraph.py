...
class ModuleGraph(ObjectGraph):
    ...
    def _find_head_package(
        ...
        if target_package is not None:
            self.msgout(
                4, "find_head_package ->", (target_package, target_module_tailname)
            )
    ...
    def _safe_import_module(self, module_partname, module_name, parent_module):
        ...
        if parent_module is not None:
            self.msg(
                4, "safe_import_module create reference", module, "->", parent_module
            )
        ...
        if loader is BUILTIN_MODULE:
            ...
            if src is not None:
                ...
                try:
                    ...
                    self.msg(
                        2,
                        "load_module: InvalidCompiledModule, " "Cannot load code",
                        pathname,
                        exc,
                    )
    ...
    def report(self):
        ...
        for m in sorted(self.iter_graph(), key=lambda n: n.graphident):
            if isinstance(m, AliasNode):
                ...
                print(
                    "%-15s %-25s %s"
                    % (type(m).__name__, m.graphident, m.filename or "")
                )
