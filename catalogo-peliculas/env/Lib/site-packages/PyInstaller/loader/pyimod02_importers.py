...
if sys.platform == "darwin" and _RESOLVED_TOP_LEVEL_DIRECTORY.endswith(
    "Contents/Frameworks"
):
...
class PyiFrozenImporter:
    ...
    def __init__(self, path):
        ...
        try:
            relative_path = self._compute_relative_path(
                resolved_path, _RESOLVED_TOP_LEVEL_DIRECTORY
            )
        except Exception:
            if _is_macos_app_bundle:
                relative_path = self._compute_relative_path(
                    resolved_path, _ALTERNATIVE_TOP_LEVEL_DIRECTORY
                )
            else:
                raise
    ...
    def find_spec(self, fullname, target=None):
        ...
        """
        trace(
            f"{self}: find_spec: called with fullname={fullname!r}, target={fullname!r}"
        )
        ...
        if entry_data is None:
            ...
            if self.fallback_finder is not None:
                ...
                trace(
                    f"{self}: find_spec: fallback finder returned spec: {fallback_spec!r}."
                )
    ...
    def get_filename(self, fullname):
        ...
        if entry_data is None:
            raise ImportError(
                f"Module {fullname!r} not found in PYZ archive (entry {pyz_entry_name!r})."
            )
        typecode = entry_data[0]
        ...
        if typecode == pyimod01_archive.PYZ_ITEM_PKG:
            return os.path.join(
                sys._MEIPASS, pyz_entry_name.replace(".", os.path.sep), "__init__.pyc"
            )
        elif typecode == pyimod01_archive.PYZ_ITEM_MODULE:
            return os.path.join(
                sys._MEIPASS, pyz_entry_name.replace(".", os.path.sep) + ".pyc"
            )
...
class PyiFrozenResourceReader:
    ...
    def __init__(self, importer, name):
        ...
        if self.importer.is_package(
            name
        ):  # covers both normal packages and PEP-420 namespace packages
...
def install():
    ...
    try:
        pyz_archive = pyimod01_archive.ZlibArchiveReader(
            sys._pyinstaller_pyz, check_pymagic=True
        )
    except Exception as e:
    ...
    for idx, entry in enumerate(sys.path_hooks):
        if getattr(entry, "__name__", None) == "zipimporter":
            trace(
                f"PyInstaller: inserting our finder hook at index {idx + 1} in sys.path_hooks."
            )
