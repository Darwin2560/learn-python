...
def _autodetect_used_backends(hook_api):
    ...
    for candidate in candidates:
        try:
            ...
            _check_mpl_backend_importable(
                module_name
            )  # NOTE: uses an isolated sub-process.
        except Exception:
            continue
        return [candidate]
    ...
    logger.info(
        "None of the backend candidates could be imported; falling back to headless Agg!"
    )
    return ["Agg"]
...
def _collect_all_importable_backends(hook_api):
    ...
    backend_names = (
        _list_available_mpl_backends()
    )  # NOTE: retrieved in an isolated sub-process.
    ...
    for backend_name in backend_names:
        ...
        try:
            ...
            logger.info(
                "  Matplotlib backend %r: ignored due to import error", backend_name
            )
            continue
...
def hook(hook_api):
    ...
    if backends_method == "auto":
        logger.info(
            "Matplotlib backend selection method: automatic discovery of used backends"
        )
        backend_names = _autodetect_used_backends(hook_api)
    elif backends_method == "all":
        logger.info(
            "Matplotlib backend selection method: collection of all importable backends"
        )
