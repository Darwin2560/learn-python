...
class TclTkInfo:
    ...
    def _load_tcl_tk_info(self):
        ...
        if tcl_tk_info is None:
            logger.debug(
                "%s: failed to obtain Tcl/Tk info: tkinter/_tkinter could not be imported.",
                self,
            )
            return
        ...
        if self.is_macos_system_framework:
            logger.info(
                "%s: using macOS system Tcl/Tk framework - not collecting data files.",
                self,
            )
        else:
            ...
            if os.path.isdir(self.tcl_data_dir):
                ...
                logger.warning(
                    "%s: Tcl library/data directory %r does not exist!",
                    self,
                    self.tcl_data_dir,
                )
            ...
            if os.path.isdir(self.tk_data_dir):
                ...
                logger.warning(
                    "%s: Tk library/data directory %r does not exist!",
                    self,
                    self.tk_data_dir,
                )
            ...
            if os.path.isdir(self.tcl_module_dir):
                ...
                logger.warning(
                    "%s: Tcl module directory %r does not exist!",
                    self,
                    self.tcl_module_dir,
                )
    ...
    def _find_tcl_tk_shared_libraries(tkinter_ext_file):
        ...
        for _, lib_path in bindepend.get_imports(
            tkinter_ext_file
        ):  # (name, fullpath) tuple
            if lib_path is None:
