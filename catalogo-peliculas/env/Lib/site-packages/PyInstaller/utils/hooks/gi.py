...
class GiModuleInfo:
    def __init__(self, module, version, hook_api=None):
        ...
        def _get_module_info(module, version):
            ...
            sharedlibs = (
                [lib.strip() for lib in sharedlibs.split(",")] if sharedlibs else []
            )
    ...
    def collect_typelib_data(self):
        ...
        for resolved_lib in resolved_libs:
            logger.debug(
                "Collecting shared library %s at %s", resolved_lib[0], resolved_lib[1]
            )
...
def gir_library_path_fix(path):
    ...
    if compat.is_darwin:
        ...
        command = subprocess.Popen(
            (
                "g-ir-compiler",
                os.path.join(CONF["workpath"], gir_name),
                "-o",
...
def compile_glib_schema_files(datas_toc, workdir, collect_source_files=False):
    ...
    try:
        ...
        # minimize damage.
        logger.warning(
            "Failed to recompile GLib schemas! Returning collected files as-is!",
            exc_info=True,
        )
    ...
    if old_compiled_file_hash is not None:
        ...
        if new_compiled_file_hash == old_compiled_file_hash:
            os.utime(
                compiled_file,
                ns=(
                    old_compiled_file_stat.st_atime_ns,
                    old_compiled_file_stat.st_mtime_ns,
                ),
            )

    # Add the resulting gschemas.compiled file to the output TOC
    output_toc.append(
        (str(SCHEMA_DEST_DIR / compiled_file.name), str(compiled_file), "DATA")
    )
    ...
    if collect_source_files:
        for schema_file in schema_files:
            output_toc.append(
                (str(SCHEMA_DEST_DIR / schema_file.name), str(schema_file), "DATA")
            )
