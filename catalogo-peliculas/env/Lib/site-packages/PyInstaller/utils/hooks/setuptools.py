...
def _retrieve_setuptools_info():
    ...
    if version >= (71, 0):
        ...
        vendored_data += collect_data_files(
            "setuptools._vendor", includes=["**/*.dist-info"]
        )
...
class SetuptoolsInfo:
    ...
    def _load_setuptools_info(self):
        ...
        if setuptools_info is None:
            logger.debug(
                "%s: failed to obtain setuptools info: setuptools could not be imported.",
                self,
            )
            return
    ...
    def get_vendored_aliases(self, module_name):
        ...
        return self._create_vendored_aliases(
            vendored_name, module_name, self.vendored_modules
        )

    def get_distutils_aliases(self):
        vendored_name = "setuptools._distutils"
        return self._create_vendored_aliases(
            vendored_name, "distutils", self.distutils_modules
        )
...
def pre_safe_import_module(api):
    ...
    for aliased_name, real_vendored_name in setuptools_info.get_vendored_aliases(
        module_name
    ):
