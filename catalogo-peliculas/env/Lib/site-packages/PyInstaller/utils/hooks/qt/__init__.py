...
class QtLibraryInfo:
    ...
    @staticmethod
    def _use_new_layout(
        package_basename: str, version: str, fallback_value: bool
    ) -> bool:
        ...
        if hooks.check_requirement(f"{package_basename}_commercial"):
            return hooks.check_requirement(
                f"{package_basename}_commercial >= {version}"
            )
    ...
    def _load_qt_info(self):
        ...
        def _read_qt_library_info(package):
            ...
            if hasattr(QLibraryInfo, "path"):
                ...
                path_names = [
                    x for x in dir(QLibraryInfo.LibraryPath) if x.endswith("Path")
                ]
                location = {
                    x: QLibraryInfo.path(getattr(QLibraryInfo.LibraryPath, x))
                    for x in path_names
                }
            else:
                ...
                location = {
                    x: QLibraryInfo.location(getattr(QLibraryInfo, x))
                    for x in path_names
                }
        ...
        self.qt_inside_package = (
            self.package_location == resolved_qt_prefix_path
            or self.package_location  # PySide2 and PySide6 Windows PyPI wheels
            in resolved_qt_prefix_path.parents
        )
    ...
    def _load_module_info(self):
        ...
        for entry in _modules_info.QT_MODULES_INFO:
            ...
            if entry.bindings:
                applicable_bindings = _modules_info.process_namespace_strings(
                    entry.bindings
                )
            ...
            info_entry = QtModuleInfo(
                module=entry.module,
                shared_lib=f"Qt{self.qt_major}{entry.shared_lib}"
                if entry.shared_lib
                else None,
    ...
    def collect_module(self, module_name):
        ...
        while imported_libraries:
            (
                imported_lib_name,
                imported_lib_path,
            ) = imported_libraries.pop()  # (name, fullpath) tuple

            # Skip unresolved libraries
            if imported_lib_path is None:
                logger.debug(
                    "%s: ignoring unresolved library import %r", self, imported_lib_name
                )
                continue
            ...
            if lib_name in self.shared_libraries:
                logger.debug(
                    "%s: collecting Qt module associated with %r.", self, lib_name
                )
        ...
        for translation_base_name in translation_base_names:
            ...
            translation_pattern = os.path.join(
                translation_src, translation_base_name + "_*.qm"
            )
            translation_files = glob.glob(translation_pattern)
            if translation_files:
                datas += [
                    (translation_file, translation_dst)
    ...
    def collect_plugins(self, plugin_type):
        ...
        logger.debug(
            "%s: found plugin files for plugin type %r: %r",
            self,
            plugin_type,
            plugin_files,
        )
    ...
    def _validate_plugin_dependencies(self, plugin_file):
        ...
        imported_libraries = bindepend.get_imports(
            plugin_file, search_paths=[self.qt_lib_dir]
        )
    ...
    def collect_qtnetwork_files(self):
        ...
        if not openssl_enabled or openssl_version == 0:
            logger.debug(
                "%s: QtNetwork: does not support SSL or does not use OpenSSL.", self
            )
            return []
    ...
    def _collect_qtnetwork_openssl_windows(self, openssl_version):
        ...
        if openssl_version >= 0x10000000 and openssl_version < 0x10100000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.0.x DLLs: %r", self, dll_names
            )
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.1.x DLLs: %r", self, dll_names
            )
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 3.0.x DLLs: %r", self, dll_names
            )
        else:
            dll_names = []  # Nothing to search for
            logger.warning(
                "%s: QtNetwork: unsupported OpenSSL version: %X", self, openssl_version
            )
        ...
        for dll in dll_names:
            ...
            dll_file_path = bindepend.resolve_library_path(
                dll, search_paths=[self.qt_lib_dir]
            )
    ...
    def _collect_qtnetwork_openssl_macos(self, openssl_version):
        ...
        if openssl_version >= 0x10100000 and openssl_version < 0x30000000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.1.x dylibs: %r", self, dylib_names
            )
        elif openssl_version >= 0x30000000 and openssl_version < 0x40000000:
            # OpenSSL 3.0.x
            dylib_names = (
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 3.0.x dylibs: %r", self, dylib_names
            )
        else:
    ...
    def _collect_qtnetwork_openssl_linux(self, openssl_version):
        ...
        if openssl_version >= 0x10000000 and openssl_version < 0x10100000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.0.x shared libraries: %r",
                self,
                shlib_names,
            )
        elif openssl_version >= 0x10100000 and openssl_version < 0x30000000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.1.x shared libraries: %r",
                self,
                shlib_names,
            )
        elif openssl_version >= 0x30000000 and openssl_version < 0x40000000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 3.0.x shared libraries: %r",
                self,
                shlib_names,
            )
        else:
    ...
    def collect_qtqml_files(self):
        ...
        for qmldir_file in sorted(qmldir_files):
            ...
            try:
                ...
                datas += [
                    (str(src_file), str(_compute_dest_dir(src_file)))
                    for src_file in plugin_datas
                ]
            except Exception:
                logger.warning(
                    "%s: failed to process QML plugin directory %s",
                    self,
                    plugin_dir,
                    exc_info=True,
                )
    ...
    def _process_qml_plugin(self, qmldir_file):
        ...
        for line in contents.splitlines():
            ...
            if plugin_path is not None:
                raise Exception(
                    f"Non-empty plugin path ({plugin_path!r} is not supported yet!"
                )
            ...
            if not plugin_file.is_file():
                logger.warn(
                    "%s: QML plugin binary %r does not exist!", str(plugin_file)
                )
                continue
    ...
    def collect_qtwebengine_files(self):
        ...
        if is_macos_framework:
            ...
            bundled_qt_libs = (
                pathlib.Path(self.package_location)
                in pathlib.Path(src_framework_path).parents
            )
            if bundled_qt_libs:
                dst_framework_path = os.path.join(
                    rel_data_path, "lib/QtWebEngineCore.framework"
                )
            else:
                dst_framework_path = (
                    "QtWebEngineCore.framework"  # In top-level directory
                )
            ...
            versions = sorted(
                [
                    ...
                    for version in os.listdir(
                        os.path.join(src_framework_path, "Versions")
                    )
                    if version != "Current"
                ]
            )
            if len(versions) == 0:
                raise RuntimeError(
                    "Could not determine version of the QtWebEngineCore.framework!"
                )
            elif len(versions) > 1:
                logger.warning(
            ...
            src_helpers_path = os.path.join(
                src_framework_path, "Versions", version, "Helpers"
            )
            dst_helpers_path = os.path.join(
                dst_framework_path, "Versions", version, "Helpers"
            )
            ...
            src_resources_path = os.path.join(
                src_framework_path, "Versions", version, "Resources"
            )
            dst_resources_path = os.path.join(
                dst_framework_path, "Versions", version, "Resources"
            )
            ...
            datas += hooks.collect_system_data_files(
                src_resources_path, dst_resources_path
            )
            ...
            binaries.append(
                (
                    os.path.join(
                        self.location["LibraryExecutablesPath"], "QtWebEngineProcess*"
                    ),
                    dest,
                )
            )
            ...
            qt_conf_file = os.path.join(
                self.location["LibraryExecutablesPath"], "qt.conf"
            )
        ...
        if compat.is_linux:
            ...
            module_file = hooks.get_module_file_attribute(
                self.namespace + ".QtWebEngineCore"
            )
            for lib_name, lib_path in bindepend.get_imports(
                module_file
                ...
                for subdir in NSS_LIB_SUBDIRS:
                    for lib_name in NSS_LIBS:
                        lib_file = os.path.normpath(
                            os.path.join(libnss_dir, subdir, lib_name)
                        )
                        if os.path.isfile(lib_file):
                            logger.debug(
                                "%s: collecting NSS library: %r", self, lib_file
                            )
...
def exclude_extraneous_qt_bindings(hook_name, qt_bindings_order=None):
    ...
    if len(available_qt_bindings) == 1:
        logger.info(
            "%s: selected %r as the only available Qt bindings.",
            hook_name,
