...
def is_package(module_name: str):
    ...
    def _is_package(module_name: str):
        ...
        try:
            ...
            return bool(spec.submodule_search_locations) or spec.origin.endswith(
                "__init__.py"
            )
        except Exception:
            return False
...
def _collect_submodules(name, on_error):
    ...
    logger.debug(
        "collect_submodules - scanning (sub)package %s in location(s): %s", name, paths
    )
...
def collect_data_files(
    ...
    if not include_py_files:
        ...
        excludes += [
            "**/*" + s for s in compat.ALL_SUFFIXES if s not in {".py", ".pyc"}
        ]
...
def collect_all(
    ...
    hiddenimports = collect_submodules(
        package_name, on_error=on_error, filter=filter_submodules
    )
...
def collect_delvewheel_libs_directory(
    package_name, libdir_name=None, datas=None, binaries=None
):
    """
