...
def _find_element_by_tag(root, tag):
    ...
    if len(elements) > 1:
        raise ValueError(
            f"Expected a single {tag!r} element, found {len(elements)} element(s)!"
        )
    if not elements:
        return None
    return elements[0]
...
def _set_execution_level(
    manifest_dom, root_element, uac_admin=False, uac_uiaccess=False
):
    """
    ...
    # <requestedPrivileges>
    requested_privileges_element = _find_element_by_tag(
        security_element, "requestedPrivileges"
    )
    ...
    if not requested_execution_level_element:
        requested_execution_level_element = manifest_dom.createElement(
            "requestedExecutionLevel"
        )
...
def _ensure_common_controls_dependency(manifest_dom, root_element):
    ...
    for dependency_element in dependency_elements:
        # <dependentAssembly>
        dependent_assembly_element = _find_element_by_tag(
            dependency_element, "dependentAssembly"
        )
        # <assemblyIdentity>
...
def write_manifest_to_executable(filename, manifest_xml):
    ...
    winresource.add_or_update_resource(
        filename, manifest_xml, RT_MANIFEST, names, languages
    )
