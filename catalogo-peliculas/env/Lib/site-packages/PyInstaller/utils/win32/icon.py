...
def CopyIcons_FromIco(dstpath, srcpath, id=1):
    ...
    for i, f in enumerate(icons):
        ...
        logger.debug(
            "Writing RT_GROUP_ICON %d resource with %d bytes", i + 1, len(data)
        )
...
def CopyIcons(dstpath, srcpath):
    ...
    if len(srcpath) > 1:
        ...
        for s in srcpath:
            srcs.append(
                normalize_icon_type(s[0], ("ico",), "ico", config.CONF["workpath"])
            )
    ...
    srcpath = normalize_icon_type(
        srcpath, ("exe", "ico"), "ico", config.CONF["workpath"]
    )
