...
class AppBuilder:
    ...
    def _test_executables(self, name, args, runtime, run_from_path, **kwargs):
        ...
        for exe in exes:
            ...
            if retcode != kwargs.get("retcode", 0):
                pytest.fail(
                    "Running exe %s failed with return-code %s." % (exe, retcode)
                )
    ...
    def _run_executable(self, prog, args, run_from_path, runtime):
        ...
        if run_from_path:
            ...
            prog_env["PATH"] = os.pathsep.join(
                [prog_env.get("PATH", ""), os.path.dirname(prog)]
            )
...
def compiled_dylib(tmpdir, request):
    ...
    try:
        if is_win:
            ...
            ret = subprocess.call(
                "install_name_tool -id %s ctypes_dylib.dylib" % (id_dylib,), shell=True
            )
        else:
            tmp_data_dir = tmp_data_dir.join("ctypes_dylib.so")
            ret = subprocess.call(
                "gcc -fPIC -shared ctypes_dylib.c -o ctypes_dylib.so", shell=True
            )
