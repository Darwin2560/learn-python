...
def fix_exe_for_code_signing(filename):
    ...
    page_size = (
        0x4000 if _get_arch_string(header.header).startswith("arm64") else 0x1000
    )
...
def convert_binary_to_thin_arch(filename, thin_arch, output_filename=None):
    ...
    p = subprocess.run(
        cmd_args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding="utf-8"
    )
    if p.returncode:
        raise SystemError(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for arch in archs:
                ...
                convert_binary_to_thin_arch(
                    filename, arch, output_filename=slice_filename
                )
...
def _set_dylib_dependency_paths(filename, target_rpath):
    ...
    if dylib_id:
        normalized_dylib_id = str(
            pathlib.PurePath("@rpath") / pathlib.PurePath(dylib_id).name
        )
...
def collect_files_from_framework_bundles(collected_files):
    ...
    framework_files = (
        set()
    )  # Additional entries for collected files. Use set for de-duplication.
    ...
    for dest_name, src_name, typecode in collected_files:
        ...
        src_path = pathlib.Path(
            src_name
        )  # /src/path/to/<name>.framework/Versions/<version>/<name>
        ...
        if not info_plist_src.is_file():
            ...
            info_plist_src_top = (
                src_path.parent.parent.parent / "Resources" / "Info.plist"
            )
        ...
        framework_files.add(
            (
                str(dest_path.parent.parent / "Current"),
                str(dest_path.parent.name),
                "SYMLINK",
            )
        )
        ...
        )  # Top-level .framework directory path.
