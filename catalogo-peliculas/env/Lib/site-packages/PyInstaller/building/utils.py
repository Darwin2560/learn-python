...
from PyInstaller.compat import EXTENSION_SUFFIXES, is_darwin, is_linux, is_win
...
def _check_guts_toc(attr_name, old_toc, new_toc, last_build):
    ...
    """
    return _check_guts_eq(
        attr_name, old_toc, new_toc, last_build
    ) or _check_guts_toc_mtime(attr_name, old_toc, new_toc, last_build)
...
def process_collected_binary(
    ...
    if (use_upx or use_strip) and is_linux:
        ...
        if hmac_path.is_file():
            logger.info(
                "Disabling UPX and/or strip for %s due to accompanying .hmac file!",
                src_name,
            )
            use_upx = use_strip = False
        elif chk_path.is_file():
            logger.info(
                "Disabling UPX and/or strip for %s due to accompanying .chk file!",
                src_name,
            )
            use_upx = use_strip = False
    ...
    if is_darwin:
        try:
            osxutils.binary_to_target_arch(
                cached_name, target_arch, display_name=src_name
            )
            # osxutils.remove_signature_from_binary(cached_name)  # Disabled as per comment above.
            target_rpath = str(
                pathlib.PurePath(
                    "@loader_path",
                    *[".." for level in pathlib.PurePath(dest_name).parent.parts],
                )
            )
            ...
            if strict_arch_validation:
                raise
            logger.debug(
                "File %s failed optional architecture validation - collecting as-is!",
                src_name,
            )
...
def _check_path_overlap(path):
    ...
    if CONF["workpath"].startswith(path):
        logger.error(
            'Specfile error: The output path "%s" contains WORKPATH (%s)',
            path,
            CONF["workpath"],
        )
        specerr += 1
    if CONF["specpath"].startswith(path):
        logger.error(
            'Specfile error: The output path "%s" contains SPECPATH (%s)',
            path,
            CONF["specpath"],
        )
        specerr += 1
...
def format_binaries_and_datas(binaries_or_datas, workingdir=None):
    ...
    for src_root_path_or_glob, trg_root_dir in binaries_or_datas:
        ...
        if os.path.isabs(trg_root_dir):
            raise InvalidSrcDestTupleError(
                (src_root_path_or_glob, trg_root_dir),
                "DEST_DIR must be a relative path!",
            )
        ...
        if not src_root_paths:
            raise SystemExit(
                f"Unable to find {src_root_path_or_glob!r} when adding binary and data files."
            )
        ...
        for src_root_path in src_root_paths:
            if os.path.isfile(src_root_path):
                ...
                for src_dir, src_subdir_basenames, src_file_basenames in os.walk(
                    src_root_path
                ):
                    ...
                    trg_dir = os.path.normpath(
                        os.path.join(
                            trg_root_dir, os.path.relpath(src_dir, src_root_path)
                        )
                    )
...
def strip_paths_in_code(co, new_filename=None):
    ...
    replace_paths = sorted(
        (os.path.join(f, "") for f in replace_paths), key=len, reverse=True
    )
    ...
    consts = tuple(
        ...
        if isinstance(const_co, code_func)
        else const_co
...
def compile_pymodule(name, src_path, workpath, optimize, code_cache=None):
    ...
    if code_object is None:
        ...
        if ext == ".py":
            ...
            raise ValueError(
                f"Invalid python module file {src_path}; unhandled extension {ext}!"
            )
...
def postprocess_binaries_toc_pywin32_anaconda(binaries):
    ...
    for dest_name, src_name, typecode in binaries:
        ...
        dest_path = pathlib.PurePath(dest_name)
        if (
            dest_path.name.lower() in DLL_CANDIDATES
            and dest_path.parent in DUPLICATE_DIRS
        ):
