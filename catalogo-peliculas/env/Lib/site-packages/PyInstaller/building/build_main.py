...
def discover_hook_directories():
    ...
    entry_points = importlib_metadata.entry_points(
        group="pyinstaller40", name="hook-dirs"
    )
...
def find_binary_dependencies(binaries, import_packages):
    ...
    if compat.is_win:
        ...
        extra_libdirs.append(
            os.path.dirname(getattr(sys, "_base_executable", sys.executable))
        )
    ...
    if compat.is_win:
        ...
        def process_search_paths():
            ...
            path_additions = [
                path for path in modified_path if path and path not in orig_path
            ]
        ...
        suppressed_imports = [
            package for package in _QT_BINDINGS if package not in qt_packages
        ]
        ...
        with isolated.Python() as child:
            ...
            added_dll_directories, added_path_directories = child.call(
                process_search_paths
            )

        # Process extra search paths...
        logger.info(
            "Extra DLL search directories (AddDllDirectory): %r", added_dll_directories
        )
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        if self.optimize not in {0, 1, 2}:
            raise ValueError(
                f"Unsupported bytecode optimization level: {self.optimize!r}"
            )
        ...
        if binaries:
            ...
            self._input_binaries = [
                ...
                for dest_name, src_name in format_binaries_and_datas(
                    binaries, workingdir=spec_dir
                )
            ]
    ...
    def assemble(self):
        ...
        if is_darwin and osxutils.is_framework_bundle_lib(python_lib):
            ...
            dst_path = pathlib.PurePath(
                src_path.relative_to(src_path.parent.parent.parent.parent)
            )
            self.binaries.append((str(dst_path), str(src_path), "BINARY"))
            self.binaries.append(
                (os.path.basename(python_lib), str(dst_path), "SYMLINK")
            )
        else:
        ...
        logger.info(
            "Performing binary vs. data reclassification (%d entries)",
            len(combined_toc),
        )
        ...
        for name, co in ctypes_code_objs.items():
            ...
            logger.debug(
                "Scanning %s for ctypes-based references to shared libraries", name
            )
            try:
                ...
                for dest_name, src_name, typecode in set(ctypes_binaries):
                    ...
                    if bindepend.classify_binary_vs_data(src_name) not in (
                        None,
                        "BINARY",
                    ):
                        logger.warning(
                            "Ignoring %s found via ctypes - not a valid binary!",
                            src_name,
                        )
                        continue
                    self.binaries.append((dest_name, src_name, typecode))
        ...
        optim_typecode = {0: "PYSOURCE", 1: "PYSOURCE-1", 2: "PYSOURCE-2"}[
            self.optimize
        ]
        program_toc = [
            (name, src_path, optim_typecode) for name, src_path, typecode in program_toc
        ]
        ...
        for idx, (dest, source, typecode) in enumerate(self.binaries):
            ...
            if src_parent == "lib-dynload" and not os.path.dirname(
        ...
        logger.debug(
            "Module collection settings: %r", self.graph._module_collection_mode
        )
        ...
        for name, src_path, typecode in pure_pymodules_toc:
            ...
            if _ModuleCollectionMode.PYZ in collect_mode:
                optim_typecode = {0: "PYMODULE", 1: "PYMODULE-1", 2: "PYMODULE-2"}[
                    optim_level
                ]
        ...
        if is_linux:
            for dest_name, src_name, typecode in self.binaries:
                ...
                if src_hmac_path.is_file():
                    dest_hmac_path = pathlib.PurePath(dest_name).with_name(
                        src_hmac_path.name
                    )
                ...
                src_hmac_path = (
                    src_lib_path.parent / "fipscheck" / f"{src_lib_path.name}.hmac"
                )
                if src_hmac_path.is_file():
                ...
                if src_chk_path.is_file():
                    dest_chk_path = pathlib.PurePath(dest_name).with_name(
                        src_chk_path.name
                    )
        ...
        if is_darwin:
            ...
            for dest_name, src_name, typecode in self.binaries:
                try:
                    ...
                    logger.warning(
                        "Failed to query macOS SDK version of %r!",
                        src_name,
                        exc_info=True,
                    )
                    binaries_with_invalid_sdk.append(
                        (dest_name, src_name, "unavailable")
                    )
                    continue
    ...
    def _write_warnings(self):
        ...
        def dependency_description(name, dep_info):
            if not dep_info or dep_info == "direct":
                ...
                imptype = (
                    dep_info.conditional
                    + 2 * dep_info.function
                    + 4 * dep_info.tryexcept
                )
        ...
        with open(CONF["warnfile"], "w", encoding="utf-8") as wf:
            ...
            for n, p, status in miss_toc:
                ...
                print(
                    ...
                    "- imported by",
                    ", ".join(
                        dependency_description(name, data) for name, data in importers
                    ),
                    file=wf,
                )
