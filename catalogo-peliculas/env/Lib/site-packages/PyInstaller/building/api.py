...
if is_win:
    from PyInstaller.utils.win32 import (
        icon,
        versioninfo,
        winmanifest,
        winresource,
    ...
    def assemble(self):
        ...
        for entry in self.toc:
            ...
            if name not in self.code_dict:
                ...
                optim_level = {"PYMODULE": 0, "PYMODULE-1": 1, "PYMODULE-2": 2}[
                    typecode
                ]
                try:
                    self.code_dict[name] = get_code_object(
                        name, src_path, optimize=optim_level
                    )
                except SyntaxError:
        ...
        self.code_dict = {
            name: strip_paths_in_code(code) for name, code in self.code_dict.items()
        }
...
class PKG(Target):
    ...
    def assemble(self):
        ...
        bootstrap_toc = (
            []
        )  # TOC containing bootstrap scripts and modules, which must not be sorted.
        archive_toc = (
            []
        )  # TOC containing all other elements. Sorted to enable reproducible builds.
        ...
        for dest_name, src_name, typecode in self.toc:
            ...
            if typecode not in {"OPTION", "DEPENDENCY", "SYMLINK"}:
                ...
                if pathlib.Path(src_name).resolve() == pkg_file:
                    raise ValueError(
                        f"Trying to collect PKG file {src_name} into itself!"
                    )
        ...
        CArchiveWriter(
            self.name, bootstrap_toc + archive_toc, pylib_name=self.python_lib_name
        )

        logger.info(
            "Building PKG (CArchive) %s completed successfully.",
...
class EXE(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.contents_directory:
            self.toc.append(
                ("pyi-contents-directory " + self.contents_directory, "", "OPTION")
            )

        if self.hide_console:
            # Validate the value
            _HIDE_CONSOLE_VALUES = {
                "hide-early",
                "minimize-early",
                "hide-late",
                "minimize-late",
        ...
        if is_win:
            ...
            if self.versrsrc:
                if isinstance(self.versrsrc, versioninfo.VSVersionInfo):
                    ...
                    self.versrsrc = versioninfo.load_version_info_from_text_file(
                        self.versrsrc
                    )
                else:
                    raise TypeError(
    ...
    def _check_guts(self, data, last_build):
        if not os.path.exists(self.name):
            logger.info(
                "Rebuilding %s because %s missing",
                self.tocbasename,
                os.path.basename(self.name),
            )
            return True
    ...
    def assemble(self):
        ...
        if is_win:
            ...
            if self.versrsrc:
                ...
                self._retry_operation(
                    versioninfo.write_version_info_to_executable,
                    build_name,
                    self.versrsrc,
                )
        ...
        if self.append_pkg:
            ...
            if not self.exclude_binaries:
                pkg_dst = os.path.join(
                    os.path.dirname(build_name), os.path.basename(self.pkgname)
                )
                logger.info(
                    "Copying stand-alone PKG archive from %s to %s",
                    self.pkg.name,
                    pkg_dst,
                )
        ...
        if is_win:
            ...
            osxutils.sign_binary(
                build_name, self.codesign_identity, self.entitlements_file
            )
    ...
    def _copy_windows_resource(self, build_name, resource_spec):
        ...
        if is_pe_file:
            ...
            try:
                ...
                raise IOError(
                    f"Failed to copy resources from PE file {src_filename!r}"
                ) from e
        else:
            ...
            resource_lang = (
                _to_int(resource[3]) if len(resource) >= 4 else 0
            )  # LANG_NEUTRAL
    ...
    def _retry_operation(func, *args, max_attempts=20):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not _is_allowed_exception(e) and not _is_allowed_exception(
                    e.__context__
                ):
                    raise
...
class COLLECT(Target):
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in self.toc:
            ...
            if typecode not in {"DEPENDENCY", "SYMLINK"} and not os.path.exists(
                src_name
            ):
            ...
            if os.pardir in os.path.normpath(dest_name).split(os.sep) or os.path.isabs(
                dest_name
            ):
                raise SystemExit(
                    "Security-Alert: attempting to store file outside of the dist directory: %r. Aborting."
                    % dest_name
                )
            ...
            if typecode in ("EXECUTABLE", "PKG"):
                ...
                dest_path = os.path.join(
                    self.name, self.contents_directory or "", dest_name
                )
        ...
        for entry in toc:
            ...
            if typecode == "SYMLINK":
                ...
                if key not in self._symlinks:
                    ...
                    logger.debug(
                        "Keeping symbolic link %r entry in original TOC.", entry
                    )
                    ...
                    logger.debug(
                        "Moving symbolic link %r entry to references TOC.", entry
                    )
            ...
            if key not in self._dependencies:
                ...
                dep_path = os.path.relpath(
                    self._dependencies[key], os.path.dirname(path_to_exe)
                )
