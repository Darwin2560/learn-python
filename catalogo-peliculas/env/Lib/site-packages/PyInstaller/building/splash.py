...
class Splash(Target):
    ...
    def __init__(self, image_file, binaries, datas, **kwargs):
        ...
        self.splash_requirements = set(
            filter(_filter_requirement, self.splash_requirements)
        )
    ...
    def _check_guts(self, data, last_build):
        ...
        if misc.mtime(self.image_file) > last_build:
            logger.info(
                "Building %s because file %s changed", self.tocbasename, self.image_file
            )
            return True
    ...
    def _check_tcl_tk_compatibility():
        ...
        if is_darwin and tcltk_info.is_macos_system_framework:
            ...
            raise SystemExit(
                "The splash screen feature does not support macOS system framework version of Tcl/Tk."
            )
    ...
    def generate_script(self):
        ...
        script = splash_templates.build_script(
            text_options=d, always_on_top=self.always_on_top
        )
        ...
        if self.minify_script:
            ...
            script = "\n".join(
                ...
                if not line.startswith("#") and line  # documentation  # empty lines
            )
