...
class BUNDLE(Target):
    ...
    def _process_bundle_toc(self, toc):
        ...
        for directory_path, directory_type in directory_types.items():
            ...
            logger.debug(
                "Cross-linking directory %r of type %r", directory_path, directory_type
            )
        ...
        for orig_dest_name, src_name, typecode in toc:
            ...
            if typecode == "SYMLINK" and requires_crosslink:
                bundle_toc.append(
                    (os.path.join(file_base_dir, orig_dest_name), src_name, typecode)
                )
                bundle_toc.append(
                    (
                        os.path.join(crosslink_base_dir, orig_dest_name),
                        src_name,
                        typecode,
                    )
                )
        ...
        for dest_name, src_name, typecode in bundle_toc:
            ...
            if sanitized_dest_path != dest_path:
                logger.debug(
                    "Sanitizing dest path: %r -> %r", dest_name, sanitized_dest_name
                )
    ...
    def assemble(self):
        ...
        shutil.copyfile(
            self.icon,
            os.path.join(
                self.name, "Contents", "Resources", os.path.basename(self.icon)
            ),
        )
