...
class SourceDestAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        try:
            ...
            (separator,) = (
                m
                for m in re.finditer(rf"(^\w:[/\\])|[:{os.pathsep}]", value)
                if not m[1]
            )
        except ValueError:
        ...
        if not src or not dest:
            ...
            raise argparse.ArgumentError(
                self, "You have to specify both SOURCE and DEST"
            )
...
class _RemovedNoEmbedManifestAction(_RemovedFlagAction):
    def __call__(self, *args, **kwargs):
        ...
        raise RemovedExternalManifestError(
            "Please remove your --no-embed-manifest argument."
        )
...
def main(
    ...
    **_kwargs,
    ...
    if contents_directory:
        exe_options += "\n    contents_directory='%s'," % (
            contents_directory or "_internal"
        )
    if hide_console:
