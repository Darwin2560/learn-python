...
class ImportErrorWhenRunningHook(HookError):
    def __str__(self):
        return (
            ...
            "hooks in the manual and provide a pull-request to improve PyInstaller.".format(
...
class RemovedExternalManifestError(SystemExit):
    def __init__(self, message):
        super().__init__(
            f"Support for external executable manifest was removed in PyInstaller v6.0. {message}"
        )
...
class PythonLibraryNotFoundError(IOError):
    def __init__(self):
        super().__init__(
            _MISSING_PYTHON_LIB_MSG.format(
